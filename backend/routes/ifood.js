...
router.post('/categorias', verifyToken, async (req, res, next) => {
  try {
    ...
    const categoria = await prisma.categoriaIfood.create({
      data: { nome, descricao, ordemExibicao: ordemExibicao ? parseInt(ordemExibicao) : 0, ativo: ativo ?? true }
    });
...
router.put('/categorias/:id', verifyToken, async (req, res, next) => {
  try {
    ...
    const categoria = await prisma.categoriaIfood.update({
      ...
      data: { nome, descricao, ordemExibicao: ordemExibicao ? parseInt(ordemExibicao) : undefined, ativo }
    });
    res.json(categoria);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.post('/', verifyToken, async (req, res, next) => {
  ...
  try {
    const item = await prisma.restauranteIfood.create({
      data: {
        categoriaId: parseInt(categoriaId),
        nome, descricao, telefone, linkExterno,
