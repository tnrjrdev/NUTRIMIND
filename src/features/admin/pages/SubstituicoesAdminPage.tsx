    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full rounded bg-emerald-500 py-3 text-white">{editId ? 'Salvar item' : 'Criar item'}</button>
        </form>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setViewState('form')} className="rounded bg-emerald-500 px-4 py-2 text-sm text-white">+ Novo item</button>
      </div>
