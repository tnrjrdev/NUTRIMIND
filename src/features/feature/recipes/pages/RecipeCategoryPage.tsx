...
export function RecipeCategoryPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [itemsRes, categoriesRes] = await Promise.all([
        api.get(`/receitas?categoriaId=${categoriaId}`),
        api.get('/receitas/categorias'),
      ]);
      setItems(itemsRes.data);
      setCategories(categoriesRes.data);
    } catch {
      setError('Nao foi possivel carregar as receitas desta categoria.');
    } finally {
      setLoading(false);
    }
  }, [categoriaId]);
  ...
  const openEditModal = async (recipeId: number) => {
    try {
      const response = await api.get(`/receitas/${recipeId}`);
      const recipe = response.data as Receita;

      setEditId(recipe.id);
      setFormData({
        nome: recipe.nome || '',
        descricao: recipe.descricao || '',
        destaque: recipe.destaque ?? false,
        ativo: recipe.ativo ?? true,
      });
      setError('');
      setIsModalOpen(true);
    } catch {
      setError('Nao foi possivel carregar a receita para edicao.');
    }
  };
  ...
  const handleDelete = async (recipeId: number) => {
    ...
    try {
      ...
      setError('Nao foi possivel deletar a receita.');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(editId ? 'Nao foi possivel atualizar a receita.' : 'Nao foi possivel cadastrar a receita.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PublicScaffold
      title={currentCategory ? `Receitas - ${currentCategory.nome}` : 'Receitas'}
      eyebrow="Categoria"
    ...
    >
      <div className="space-y-6">
        <section className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 shadow-xl shadow-slate-200/50">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#71695a]">
              Busque receitas, abra os detalhes, edite ou inative itens e mantenha tudo dentro da mesma interface.
            </p>
          </div>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/50">
            ...
            <p className="mt-2 text-sm text-[#786f60]">
              {search ? 'Resultados filtrados para a sua busca.' : 'Receitas prontas para consulta e manutencao.'}
            </p>
          </div>
        </section>
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/50">
          ...
          <div className="mt-6">
            {loading ? (
              <PublicLoadingState message="Carregando receitas..." />
            ) : filteredItems.length === 0 ? (
              <PublicEmptyState message="Nenhuma receita encontrada nesta categoria." />
            ) : (
                        ...
                        >
                          ...
                          <span className="block">
                            ...
                            <span className="mt-2 block text-sm leading-6 text-[#756e60]">
                              {item.descricao || 'Abra esta receita para cadastrar ingredientes e modo de preparo.'}
                            </span>
                          </span>
                        </button>
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-[#736c5d]">
                  {currentCategory?.nome
                    ? `A receita sera vinculada automaticamente a categoria ${currentCategory.nome}.`
                    : 'A receita sera vinculada automaticamente a categoria atual.'}
                </p>
              </div>
                ...
                <div className="rounded-[24px] border border-[#e7d9aa] bg-white/70 px-4 py-4">
                  ...
                  <div className="mt-4 flex items-center justify-center gap-4 text-sm text-[#4f493e]">
                    ...
                    >
                      <span
                        className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${
                          formData.destaque ? 'left-8' : 'left-1'
                        }`}
                      />
                    </button>
                    <span>Sim</span>
