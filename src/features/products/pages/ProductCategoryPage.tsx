...
function ToggleField({ label, checked, onChange }: ToggleFieldProps) {
  return (
    <label className="inline-flex items-center gap-3 text-sm font-medium text-[#5e5648]">
      ...
      >
        <span
          className={`absolute top-1 h-5 w-5 rounded-full bg-emerald-500 transition ${checked ? 'left-8' : 'left-1'}`}
        />
      </button>
      <span>{label}</span>
    </label>
...
export function ProductCategoryPage() {
  ...
  const handleHardDelete = async (productId: number) => {
    ...
    try {
      await api.delete(`/produtos/${productId}?hard=true`);
      if (selectedId === productId) {
        setSelectedId(null);
      }
      await fetchData();
      ...
      <div className="space-y-6">
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/50">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-2 text-lg font-semibold text-slate-800">Produtos disponiveis</h3>
              </div>
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/50">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-800">Produtos da categoria</h2>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[rgba(30,25,14,0.38)] px-4 py-6 backdrop-blur-[2px] sm:px-6 sm:py-8">
          <div className="mx-auto flex min-h-full w-full max-w-2xl items-center justify-center">
            ...
            <form onSubmit={handleSubmit} className="mt-8 space-y-5">
              ...
              <div className="grid gap-4 rounded-[24px] border border-slate-200 bg-white/80 p-5">
