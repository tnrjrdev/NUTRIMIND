...
function TipIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6" aria-hidden="true">
      <path d="M12 3a6.5 6.5 0 0 0-4 11.6c.9.7 1.5 1.8 1.5 3v.4h5v-.4c0-1.2.6-2.3 1.5-3A6.5 6.5 0 0 0 12 3Z" stroke="currentColor" strokeWidth="1.8" />
...
export function DicasPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      setError('Nao foi possivel carregar as dicas.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => item.texto.toLowerCase().includes(normalizedSearch));
  }, [items, search]);
  ...
  const openEditModal = async (id: number) => {
    try {
      ...
      const item = response.data as Dica;
      setEditId(item.id);
      setFormData({
        texto: item.texto || '',
      ...
      setError('Nao foi possivel carregar a dica para edicao.');
    }
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!formData.texto.trim()) {
      setError('Informe o texto da dica.');
      return;
    }
    ...
    try {
      if (editId) {
        await api.put(`/dicas/${editId}`, payload);
      } else {
        await api.post('/dicas', payload);
      }
      await fetchItems();
      closeModal();
    } catch {
      setError(editId ? 'Nao foi possivel atualizar a dica.' : 'Nao foi possivel cadastrar a dica.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Deseja inativar esta dica?')) return;
    ...
    try {
      ...
      setError('Nao foi possivel inativar a dica.');
    }
  };
      ...
      <div className="space-y-6">
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/50">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-800">Dicas cadastradas</h2>
              <p className="mt-1 text-sm text-[#776f61]">Cadastre novas orientacoes, abra o detalhe ou ajuste as dicas existentes.</p>
            </div>
          ...
          <div className="mt-6">
            {loading ? (
              <PublicLoadingState message="Carregando dicas..." />
            ) : filteredItems.length === 0 ? (
              <PublicEmptyState message="Nenhuma dica encontrada para esta busca." />
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <button
                          type="button"
                          onClick={() => navigate(`/dicas/${item.id}`)}
                        ...
                        >
                          ...
                          <span className="block">
                            <span className="flex flex-wrap items-center gap-2">
                              <span className="text-lg font-semibold text-[#2f2f2f]">{item.texto}</span>
          ...
          <div className="mx-auto flex min-h-full w-full max-w-2xl items-center justify-center">
            <div className="my-auto w-full rounded-[32px] border border-emerald-200 bg-slate-50 p-6 shadow-2xl shadow-emerald-900/5 sm:p-8">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h4 className="mt-2 text-2xl font-semibold text-slate-800">
                    {editId ? 'Atualizar dica' : 'Cadastrar dica'}
                  </h4>
                </div>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                <div>
                  <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.14em] text-[#817661]">Digite a dica</label>
                  <input
                    required
                    value={formData.texto}
                    onChange={(event) => setFormData({ ...formData, texto: event.target.value })}
                ...
                <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
                  ...
                  >
                    <PlusIcon />
                    {saving ? 'Salvando...' : editId ? 'Atualizar dica' : 'Cadastrar dica'}
                  </button>
                </div>
              </form>
            </div>
